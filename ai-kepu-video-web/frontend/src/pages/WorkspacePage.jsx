    ...
    <div className="workspace-grid">
      ...
      <aside className={`workspace-settings${settingsOpen ? ' is-open' : ''}`} aria-label="生产设置">
        ...
        {!settingsOpen ? <div className="workspace-segment-inspector">
          ...
          <section className="workspace-setting-section"><div className="workspace-setting-heading"><strong>生成策略</strong><span>保存后作为下一批生成与失败重试的默认值</span></div><div className="workspace-runtime-grid"><label><span>提示词并发</span><input type="number" min="1" max="8" value={runtimeConfig.prompt_concurrency} onChange={event => setRuntimeConfig(current => ({ ...current, prompt_concurrency: event.target.value }))} onBlur={() => setRuntimeConfig(current => normalizeRuntimeConfig(current))} /></label><label><span>配音并发</span><input type="number" min="1" max="8" value={runtimeConfig.tts_concurrency} onChange={event => setRuntimeConfig(current => ({ ...current, tts_concurrency: event.target.value }))} onBlur={() => setRuntimeConfig(current => normalizeRuntimeConfig(current))} /></label><label><span>生图并发</span><input type="number" min="1" max="8" value={runtimeConfig.image_concurrency} onChange={event => setRuntimeConfig(current => ({ ...current, image_concurrency: event.target.value }))} onBlur={() => setRuntimeConfig(current => normalizeRuntimeConfig(current))} /></label><label><span>失败后重试</span><input type="number" min="0" max="5" value={runtimeConfig.retry_count} onChange={event => setRuntimeConfig(current => ({ ...current, retry_count: event.target.value }))} onBlur={() => setRuntimeConfig(current => normalizeRuntimeConfig(current))} /></label></div><p className="workspace-runtime-note">并发范围 1–8；Agnes 默认 8 路并发，滚动 60 秒最多派发 20 个请求。</p><button type="button" className="button button-secondary workspace-runtime-save" disabled={busyAction === 'runtime'} onClick={saveRuntimeConfig}>{busyAction === 'runtime' ? <LoaderCircle className="spin" size={15} /> : <Save size={15} />}{busyAction === 'runtime' ? '正在保存…' : '保存生成策略'}</button></section>
