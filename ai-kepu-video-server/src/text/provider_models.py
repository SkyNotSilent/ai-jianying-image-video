...
def _sync_error(kind: str, public_message: str, status_code: int):
...
def _base_parts(base_url: str):
    ...
    if not value:
        _sync_error("invalid_response", "请先填写 Base URL", 400)
    ...
    if invalid_url:
        _sync_error(
            "invalid_response",
            "Base URL 必须是 http:// 或 https:// 开头的完整地址",
            400,
        )
    if (
        parsed is None
...
def _models_url(base_url: str, adapter: str) -> str:
    parsed, path = _base_parts(base_url)
...
def _adapter_for(provider_id: str, provider: Mapping[str, Any], draft: Mapping) -> str:
...
def refresh_provider_models(
    ...
    """Fetch account models and merge them with deterministic local metadata."""

    provider_id = str(provider_id or "").strip().lower()
    provider = get_provider(provider_id)
    if provider is None:
        _sync_error("unsupported", "生文服务商不存在", 404)
    ...
    adapter = _adapter_for(provider_id, provider, draft)
    base_url = draft.get("base_url") or draft.get("api_url") or provider.get(
        "default_base_url"
    )
    api_key = str(draft.get("api_key") or draft.get("token") or "").strip()
    if adapter in {"openai", "anthropic"} and not api_key:
        _sync_error("credentials", "请先填写 API Key", 400)

    models_url = _models_url(str(base_url or ""), adapter)
    ...
    if not isinstance(status_code, int) or status_code < 200 or status_code >= 400:
        _sync_error(
    ...
    canonical_provider = (
        adapter if provider_id == "custom" else provider.get("litellm_provider")
    )
