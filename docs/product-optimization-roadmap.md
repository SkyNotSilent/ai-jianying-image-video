...
> 状态：补充评审稿 · 2026-08-18 · 本轮只形成计划，不实施产品代码
> 原方案已记录 UI 视觉方向为 **B「编辑部」**（见 3.1）；本补充稿保留该结论，但以旧版三栏工作台为不可变的信息架构基线
...
- 当前工作树已经包含尚未提交的精确素材重试改造：`task_operations`、`POST /retry-assets`、`POST /finalize`、`awaiting_finalization` 以及工作台按钮分流。Roadmap 开工前必须先把这一批改动独立验收并形成可回退基线，后续批次不得把它与视觉重构混在同一提交。
...
### 3.1 视觉方向对比（已选定：B「编辑部」）

> **2026-08-18 评审结论：选 B。** 依据 `docs/ui-direction-preview.html` 的三方向工作台示意对比。B 的基调：暖纸浅色底、衬线标题（Noto Serif SC 类）+ 无衬线正文、高对比墨色、清晰阅读层级——与"文稿驱动创作"的产品叙事一致。下表保留作为决策记录。
...
| M5 | 分镜卡片操作 | hover 抬升（`translateY(-2px)` + 阴影加深）；选中态左侧墨色描边展开 | B 方向的编辑部质感：克制、精确 |
...
**C3 · 大图预览层（Lightbox）** — 点击图片全屏查看：左右键切换分镜、显示对应画面提示词、快捷操作（重新生成/替换）。编辑部风格：深色遮罩 + 白色信息条。
...
**C6 · 空态插画卡** — 各空状态（无项目、无素材、未配置）用统一的线描风格占位插画（SVG 手绘线条，编辑部气质），替代纯文字空态。
...
- C/M 系列依赖 token 和基础组件；视觉方向 B 不得改变第 0 节确认的三栏信息架构。
...
1. UI 视觉方向沿用原文档记录的 B「编辑部」，但三栏工作台结构不变。
...
1. **B 方向最终视觉样张**：在 Phase 6 开工前，由产品负责人基于 `docs/ui-direction-preview.html` 再确认一次字体、暖纸底色和信息密度；若只调整视觉 token，不影响前述三栏结构和 P0 排期。
